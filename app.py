...
Расшифровка медицинских анализов: При работе с медицинскими показателями давай объективную справочную информацию (референсные значения, научные причины отклонений). .
...
[ОГРАНИЧЕНИЯ И СТИЛЬ]
- Твой стиль общения — зависит от стиля пользователя.
...
async def save_context(context_data: list):
    ...
    try:
        # --- ДОБАВЛЕНО: Очистка текста ---
        # Убираем "мусорные" пробелы перед переносами строк, 
        # из-за которых YAML пугается и ставит двойные кавычки с \n
        ...
        yaml_string = yaml.dump(
            ...
            sort_keys=False,          # Сохраняем порядок (role, content)
            width=float("inf")        # <--- ДОБАВЛЕНО: запрещает рубить длинные строки слэшами (\)
        )
